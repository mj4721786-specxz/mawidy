      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="bg-white border rounded-lg p-5">
            <div className="text-3xl font-bold text-[#1B1420]">{s.value}</div>
            <div className="text-sm text-gray-500 mt-1">{s.label}</div>
          </div>
        ))}
      </div>
      <a href="/admin/clinics" className="inline-block mt-8 bg-[#E2A63C] text-[#1B1420] font-bold px-5 py-2.5 rounded">
        إدارة العيادات
      </a>
    </div>
  );
